...
int main()
{
    ...
    ReadDifussionData("./Cerebro.csv", TAM_X, TAM_Y, TAM_Z, 45, 45, 45, difusion);
...
int i,j,k;
  ...
  t1=(double)tp.tv_sec+(1.e-6)*tp.tv_usec;
    ...
    for (int t = 1; t < 100; t++){
        ...
        #pragma omp parallel for collapse(3)  schedule(static) private(c,d,c_plus_i,c_minus_i,c_plus_j,c_minus_j,c_plus_k,c_minus_k,i,j,k,c_new) num_threads(4)
        for (k = 1; k < TAM_Z-2; k++)
        {
                for (j = 1; j < TAM_Y-2; j++)
                {
                    for (i = 1; i < TAM_X-2; i++)
                    {
                         c = G3D(cerebro, i, j, k);
                         d = G3D(D, i, j, k);
                         c_plus_i =G3D(cerebro,i+1,j,k);
                         c_minus_i =G3D(cerebro,i-1,j,k);
                         c_plus_j =G3D(cerebro,i,j+1,k);
                         c_minus_j =G3D(cerebro,i,j-1,k);
                         c_plus_k =G3D(cerebro,i,j,k+1);
                         c_minus_k =G3D(cerebro,i,j,k-1);
